...
{
    /// <summary>
    /// Red candle + green candle ---(4-48) --- red candle (open lower than lowest of previous two ) + green candle 
    /// High volume for first red candle
    /// Lowest 
    /// </summary>
    public class DoublePeaksTroughsRecognizerStrategyAgent : IStrategyAgent
    {
        ...
        private ILongTermParametersProvider _longTermParametersProvider;
        ...
        public DoublePeaksTroughsRecognizerStrategyAgent(StrategyOptions options,
            ILongTermParametersProvider longTermParametersProvider, 
            ILogger logger)
        {
            _options = options;
            _logger = logger;
            _options = options;
            _longTermParametersProvider = longTermParametersProvider;
            _logger = logger;            
        }
        ...
        public void UpdateLongSeries(IEnumerable<Candle> series)
        {
            ...
            _longTermParametersProvider.RecalculateDeviations(series);
            _longTermParametersProvider.RecalculateCandlesStatistics(series);

            _upParameters = _longTermParametersProvider.GetDeviations(Direction.Up);
            _downParameters = _longTermParametersProvider.GetDeviations(Direction.Down);
            _candesStatistics = _longTermParametersProvider.GetStatistics();
        }
        ...
        public bool ShouldClose(AgentContext context, out AgentActionParams orderParams)
        {
            ...
            {
                //orderParams = new AgentActionParams { CurrentPositionDirection = CurrentPositionDirection.Up, Price = 0.0 };
                //orderParams = new AgentActionParams { CurrentPositionDirection = CurrentPositionDirection.Down, Price = _usdCandles.Last().Close };
        ...
        public bool ShouldSetStopLoss(AgentContext context, out AgentActionParams orderParams)
        {
            ...
            orderParams = new AgentActionParams { Direction = context.CurrentPositionDirection.GetOpposite() };

            if (context.CurrentPositionDirection == Direction.Up)
            {
