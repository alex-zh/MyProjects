...
namespace Core
{
    ...
    ///  b)цена закрытия
    /// 2. Вычисляем пройденное растояние, значение close для текущей свечи, минус значение open для свечи period свечей назад
    /// 
    /// Формула EMA
    /// EMA (t) = EMA(t-1) + alpha * (P(t)-EMA(t-1)), alpha - коэфициент, задающий степень убывания весов.
    /// </summary>
    public class CustomVolatilityCalculator
    {
        ...
        public CustomVolatilityCalculator(Candle[] candles, int period)
        {
            _candles = candles;
            _period = period;
            Init();
