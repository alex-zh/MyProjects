...
namespace BitMexConnector.UnitTests.Parsers
{
    ...
    public class OrderBookL2ParserTests
    {
        ...
        {
            _l2Parser.Parse(JObject.Parse(TestData.OrderBookL2ParserTests.OrderBookL2_Partial_json));

            var orderBook = _dataProvider.GetOrderBook(Symbols.BitMex.XBTUSD);
        ...
        public void Parse_Partial_then_Update()
        {
            ...
            var updateJson = _testDataUpdator.GetJson(Symbols.BitMex.XBTUSD, new List<UpdateDataItem>()
            {
                new UpdateDataItem {Id = 1, Side = OrderSides.Sell, Size = 1400},
                new UpdateDataItem {Id = 2, Side = OrderSides.Sell, Size = 1500 },
                new UpdateDataItem {Id = 1, Side = OrderSides.Buy, Size = 2800},
                new UpdateDataItem {Id = 2, Side = OrderSides.Buy, Size = 3000 },
            });
            ...
            Assert.AreEqual(1, bids[0].Id); Assert.AreEqual(2800, bids[0].Size); Assert.AreEqual(12000, bids[0].Price);
            Assert.AreEqual(2, bids[1].Id); Assert.AreEqual(3000, bids[1].Size); Assert.AreEqual(11000, bids[1].Price);

            Assert.AreEqual(1, asks[0].Id); Assert.AreEqual(1400, asks[0].Size); Assert.AreEqual(12000, asks[0].Price);
            Assert.AreEqual(2, asks[1].Id); Assert.AreEqual(1500, asks[1].Size); Assert.AreEqual(11000, asks[1].Price);
        ...
        public void Parse_Partial_then_Insert_then_Update()
        {
            ...
            var insertJson = _testDataUpdator.GetJson(Symbols.BitMex.XBTUSD, new List<InsertDataItem>()
            {
                new InsertDataItem() {Id = 6, Side = OrderSides.Sell, Size = 600, Price = 6000 },
                new InsertDataItem() {Id = 7, Side = OrderSides.Sell, Size = 700, Price = 7000 }
            });
            ...
            var updateJson = _testDataUpdator.GetJson(Symbols.BitMex.XBTUSD, new List<UpdateDataItem>()
            {
                new UpdateDataItem() {Id = 2, Side = OrderSides.Sell, Size = 1150},
                new UpdateDataItem() {Id = 6, Side = OrderSides.Sell, Size = 650 },
                new UpdateDataItem() {Id = 7, Side = OrderSides.Sell, Size = 750}
            });
            ...
            Assert.AreEqual(5, bids.Count);
            Assert.AreEqual(7, asks.Count);

            var item = asks.FirstOrDefault(x => x.Id == 2);

            Assert.AreEqual(1150, item.Size);
            Assert.AreEqual(11000, item.Price);

            item = asks.FirstOrDefault(x => x.Id == 6);

            Assert.AreEqual(650, item.Size);
            Assert.AreEqual(6000, item.Price);

            item = asks.FirstOrDefault(x => x.Id == 7);
